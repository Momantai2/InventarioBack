...
async def bulk_assign(
    ...
    # Llamamos a la función que ya definiste en tu servicio
...
async def check_serie(
    ...
    """Endpoint para validación de duplicados en tiempo real."""
...
async def create_proveedor(data: ProveedorRentingCreate, service: EquipmentService = Depends(get_service)):
    # Usamos data.model_dump() para que Pydantic maneje la conversión de fechas
...
async def list_estados(service: EquipmentService = Depends(get_service)):
    """Obtiene el catálogo de estados (Disponible, Asignado, etc.)"""
